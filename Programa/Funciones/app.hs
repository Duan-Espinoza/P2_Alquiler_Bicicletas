...
separaElementos :: [[Char]] -> [a] --a seria el elemento a buscar, en el ejemplo del profesor es persona
separaElementos lista = 
    if lista == [] then 
        []
    else 
        [creaPersona(separaPorComas((head lista), ""))] ++ separaElementos(tail lista)

convierteALineas :: String -> [String]
convierteALineas texto = lines texto

leerArchivo :: FilePath -> IO [a]
leerArchivo archivo = do
    contenido <- readFile archivo
    let biciletas = separaElementos(convierteALineas contenido)
    return biciletas
